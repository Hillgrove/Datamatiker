...
namespace REST_Exercise_4.Controllers
{
    ...
    public class PokemonsController : ControllerBase
    {
        ...
        [HttpGet]
        public ActionResult<IEnumerable<Pokemon>> Get([FromHeader(Name = "amount")] int? amount, [FromQuery] string? subString)
        {
            IEnumerable<Pokemon> allPokemons;
            
            if (string.IsNullOrEmpty(subString))
            {
                allPokemons = _repository.GetAll();
            }

            else
            {
                allPokemons = _repository.GetbySubstring(subString);
            }
        ...
        // GET for Pagination: api/<PokemonsController>/paginated
        [ProducesResponseType(StatusCodes.Status200OK)]
